...
def custom_analysis_with_ai(custom_query):
    try:
        ...
        return None
if st.button("Generate Table Automatically"):
    default_prompt = (
        "Generate Python code that creates a pandas DataFrame named `df` "
    )
    ...
    with st.spinner("Generating code..."):
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are an assistant that only returns Python code. Output must define a pandas DataFrame named `df`."},
        ...
        try:
            ...
            df = local_vars.get('df', None)

            if isinstance(df, pd.DataFrame):
                st.success("Here's the table generated by the code:")
                st.dataframe(df)
            else:
                st.warning("The code did not produce a DataFrame named `df`.")
        except Exception as e:
